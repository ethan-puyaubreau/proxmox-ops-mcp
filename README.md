...
The approval message shows the tool name and every argument in full, so the approver sees exactly what will run. An action too long to fit in one Telegram message is denied without asking. If the approval request cannot be delivered, the action is denied at once instead of waiting for the timeout.
